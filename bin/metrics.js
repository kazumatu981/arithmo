    ...
    function visit(node) {
        if (
            ...
            functions.push({
                ...
                nesting,
                cyclomatic: cc,
                commentLines: fm.commentLines,
                emptyLines: fm.emptyLines,
                execLines: fm.execLines,
            });
        }
...
function toMarkdown(report, baseSrcDir) {
    ...
    const commentRate = (
        (report.metrics.commentLines / report.metrics.total) *
        100
    ).toFixed(2);
    const emptyRate = (
        (report.metrics.emptyLines / report.metrics.total) *
        100
    ).toFixed(2);
    ...
    // readability
    md += `\n## 可読性評価\n`;
    const avgFuncLines = report.functions.length
        ? report.functions.reduce((s, x) => s + x.lines, 0) /
    ...
    let overall = '普通';
    if (
        report.metrics.commentLines / report.metrics.total < 0.05 ||
        avgFuncLines >= 50 ||
        highComplex.length > 0 ||
        deepNest.length > 0
    )
        overall = '低い';
    md += `- 全体の可読性評価: ${overall}\n`;
    ...
    for (const f of files) {
        try {
            ...
            const outPath = path.join(
                outBase,
                rel.replace(/\.ts$/, '_metrics_report.md'),
            );
